...
export async function GET() {
  console.log('German 2nd Bundesliga API route called');
  
  try {
    // Path to cache file
    const cacheFilePath = path.join(process.cwd(), 'german_2bundesliga_cache.json');
    ...
    let currentRound = 8; // Starting round
    const matchesPerRound = 9;
    ...
    while (currentRound <= 34) {
      ...
      if (lastGameTime) {
        ...
        console.log(`Round ${currentRound}: Last game kicked off at ${lastGameTime.toISOString()}, ${timeSinceLastKickoff.toFixed(2)} minutes ago`);
    ...
    const normalizedMatches = normalizeMatches(
      filteredMatches,
      'german-2bundesliga',
      LEAGUE_CONFIGS.GERMAN_2BUNDESLIGA,
    ...
    console.error('Error in German 2nd Bundesliga API:', error);
